...
namespace tongtastic\AdobeSign;

use GuzzleHttp\Psr7\MultipartStream;
use tongtastic\AdobeSign\Exceptions\AdobeSignException;
use tongtastic\AdobeSign\Exceptions\AdobeSignInvalidAccessTokenException;
use tongtastic\AdobeSign\Exceptions\AdobeSignMissingRequiredParamException;
use tongtastic\AdobeSign\Exceptions\AdobeSignUnsupportedMediaTypeException;
...
class AdobeSign
{
    ...
    /**
     * @var \tongtastic\AdobeSign\AdobeSignProvider
     */
    protected $provider;
    ...
    public function getAgreements(array $query = [], array $headers = [])
    {
        ...
        $request = $this->provider->getAuthenticatedRequest(
            'GET',
            $this->provider->getBaseUri() . '/' . $this->version . "/agreements?query=$query",
            $this->accessToken, [
                'headers' => $headers
            ]
        );
